...
interface SearchUsersResult {
  users: Omit<User, "password" | "refreshToken">[];
  total: number;
}
...
export class UserModel {
  ...
  async update(id: string, userData: Partial<User>): Promise<boolean> {
    try {
      ...
      const result = await UserSchema.findByIdAndUpdate(id, updateData, {
        new: true,
      });
      return !!result;
    } catch (error) {
  ...
  async searchUsers(
    ...
    try {
      ...
      return { users: users.map((user) => this.documentToUser(user)), total };
    } catch (error) {
  ...
  async searchUsersWithFuzzy(
    query: string,
    skip: number = 0,
    limit: number = 5
  ) {
    try {
      // 1️⃣ Lấy tất cả users từ DB (chỉ lấy field cần thiết)
      const allUsers: UserDocument[] = await UserSchema.find({})
        .select("-password -refreshToken")
        .lean();

      // 2️⃣ Tạo searcher subsequence match
      const searcher = new FuzzySearch<UserDocument>(
        allUsers,
        ["username", "email"],
        {
          caseSensitive: false,
          sort: true, // kết quả tốt nhất lên đầu
        }
      );

      // 3️⃣ Thực hiện fuzzy search nếu query có giá trị
      const results: UserDocument[] = query.trim()
        ? searcher.search(query)
        : allUsers;

      // 4️⃣ Sort theo createdAt desc, fix lỗi TypeScript
      const sortedResults = results.sort((a, b) => {
        const valA = a.createdAt instanceof Date ? a.createdAt.getTime() : 0;
        const valB = b.createdAt instanceof Date ? b.createdAt.getTime() : 0;
        return valB - valA; // sort desc
      });

      // 5️⃣ Pagination
      const paginatedResults = sortedResults.slice(skip, skip + limit);

      // 6️⃣ Map sang User, loại bỏ password & refreshToken
      ...
      return { users, total: sortedResults.length };
    } catch (error) {
      console.error("Search users error:", error);
      throw error;
    }
  ...
  async advancedSearch(
    ...
    try {
      ...
      const [users, total] = await Promise.all([
        UserSchema.find(query).select(projection).skip(skip).limit(limit),
  ...
  async getSuggestions(
    ...
    try {
      ...
      const suggestions = await UserSchema.find(filter)
        .select(`${field} -_id`)
        .limit(limit)
        .lean();
      ...
      const uniqueValues = [
        ...new Set(
          suggestions
            .map((item) => item[field as keyof UserDocument])
  ...
  async searchByText(searchText: string, limit: number = 5): Promise<User[]> {
    try {
      const users = await UserSchema.find({
        $or: [
